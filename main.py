...
処理の流れ:
  ...
  4. 【二次スクリーニング】通過銘柄を詳細スコアリング → 上位銘柄を抽出
  5. 前回レポートの検証（前回上位銘柄のその後の推移を機械的に集計）
  6. レポートをターミナル出力 ＋ LINE配信（まとめカード＋評価カード＋テキスト）
...
def _build_price_map(histories):
    """取得済み銘柄の {証券コード: 最新終値} を作る（前回検証の今回株価に使用）。"""
    price_map = {}
...
def main():
    ...
    # 前回検証の「今回株価」に使う価格マップ（取得できた全銘柄の最新終値）
    ...
    # 6. レポート出力（ターミナル詳細テキスト＋LINEはカード＋テキスト）
...
def _deliver(report, flex_messages=None):
    """
    レポートをLINEへ送信する（まとめカード＋銘柄別の評価カード＋テキスト本文）。
