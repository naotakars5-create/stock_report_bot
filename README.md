...
5. レポートに以下を日本語で出力
   - 本日の市場概況
   - **本日の全体トレンド**（国内/海外/為替/物色の広がり/注目業種/TOP10平均の機械的サマリー）
   - スクリーニング結果サマリー（分析対象数 / 取得成功数 / 一次通過数 / 最終数）
   - 最終注目10銘柄を**詳しく解説**
     （銘柄名 / 証券コード / 業種 / 現在株価 / 評価点 / 想定上昇率 /
     **スコア内訳(6観点)** / **テクニカル指標** / 注目理由 / リスク / **総合コメント**）
   - 末尾に注意書き（投資助言ではない旨）
6. LINE送信は **見やすいカード（Flexメッセージ）中心**で配信（画像ホスティング不要）
   - **まとめカード**: 市場概況・全体トレンド一言・注目銘柄TOP10（評価点/想定上昇率）
   - **銘柄別の詳細評価カード（横スワイプのカルーセル）**: 各銘柄の
     スコア内訳を**横棒グラフ**で可視化＋テクニカル指標・注目理由・リスク・総評
     （カードは50KB/メッセージ上限に収まるよう自動で複数カルーセルに分割）
   - 加えて短縮テキスト（市場概況・トレンド・TOP10一覧）も送信。
     ターミナル出力は従来どおり全銘柄の詳細テキスト
...
テキストは情報量を保ちつつ縦の長さを圧縮した**コンパクト形式**（1銘柄6行）です。
LINEへはこのテキストに加え、見やすい **Flexメッセージ（まとめカード）** も送信します。

```
■ 日本株 AIレポート（東証スクリーニング版）  2026/06/25 08:00

【市場概況】
  日経平均 39,500.00(+0.85%) ｜ TOPIX(連動ETF) 2,750.00(+0.60%) ｜ ドル円 161.75(+0.12%)
  S&P500 7,358.20(-0.10%) ｜ NASDAQ 25,476.60(-0.43%)

【本日の全体トレンド】
  国内株はしっかりの展開、物色は一部銘柄に集中。
  ・国内市場: TOPIX連動ETFは前日比 +0.60% で上昇し堅調。
  ・海外市場: S&P500 -0.10% / NASDAQ -0.43%（前日の米国市場はまちまち）。
  ・物色の広がり: 全3,728銘柄中、上昇条件を満たしたのは50銘柄(1.3%)で、選別色の強い地合い。
  ・注目業種(TOP10内): 機械3、電気機器2、不動産業1。
  ・TOP10平均: 評価点 8.5 / 想定上昇率 +6.94%。

【スクリーニング】対象 3,728 → 一次通過 50 → 最終 10 件（取得: 一次3,700/二次50）

【本日の注目銘柄 TOP10】
（内訳=取得点/満点 ｜ 指標 5-25日/25-75日=移動平均かい離, 対TOPIX=20日相対pt, ボラ=日次）

■1位 荒川化学工業(4968)・化学  2,253.0円｜評価8.6/10｜想定+7.10%
  内訳 短期2.0/2.0 中期2.0/2.0 出来高1.5/1.5 相対2.0/2.0 過熱1.1/1.5 安定0.0/1.0
  指標 5-25日+30.5% / 25-75日+20.6% / 出来高2.45倍 / 対TOPIX+50.5pt / 5日+4.2% / ボラ5.1%
  理由 5日線が25日線を30.5%上回り短期上昇基調／25日線が75日線を20.6%上回り中期トレンドは上向き／…
  リスク 日次ボラティリティ5.1%と高く値動きが荒い
  総評 総合評価は高水準。特に「短期トレンド・中期トレンド・出来高・相対強さ」が強み。一方で「安定性」は弱め。
...
```
