...
5. レポートに以下を日本語で出力
   ...
   - 末尾に注意書き（売買推奨ではない旨）
6. LINE送信は **まとめカード（Flex）→ 詳細テキスト** の順で配信（画像ホスティング不要）
   - **まとめカード（1分で読める短縮版）**: タイトル / 市場概況 /
     スクリーニング上位5銘柄（評価点・事業・評価タグ） / 前回検証の簡易サマリー / 注意書き
...
詳細テキストでは、各銘柄の評価を**テキストの簡易図**で表示します（画像ではありません）。
...
- 図には主要6軸を表示し、継続性補正は「スコア内訳」に含めて表示します。
...
- **A. `company_profiles.csv` に登録がある銘柄**: その事業内容・テーマタグを使用
- **B. 未登録の銘柄**: JPXの業種名から**簡易プロフィールを自動生成**
  （事業内容は「〇〇業種に属する企業」、テーマタグは業種から推定）
...
- LINEへは「**まとめカード（1分で読める短縮版）→ 詳細テキスト**」の順で送信します。
- まとめカードには タイトル / 日付 / 市場概況 / スクリーニング上位5銘柄（評価点・事業・評価タグ） /
  前回検証の簡易サマリー / 注意書き を表示します。
...
LINEへは「まとめカード（Flex）→ 下記の詳細テキスト」の順で送信します。
