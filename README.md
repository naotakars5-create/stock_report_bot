...
5. レポートに以下を日本語で出力
   ...
   - **スクリーニング上位5銘柄** を機械的に評価
     （順位 / 銘柄名 / 証券コード / 評価点 / **事業内容** / **テーマタグ** /
     **ニュース環境** / **評価バランス図（7軸）** / **評価理由** / **スコア内訳** /
     **リスクメモ**）
     ※「想定上昇率」「上昇予想」などは出しません
...
6. LINE送信は **①まとめカード（Flex）→ ②銘柄カルーセル（横スライドカード）→ ③詳細テキスト**
   ...
   - **①まとめカード**: タイトル / 日付 / スクリーニング集計（分析対象・取得成功・一次通過・
     ...
     上位5銘柄の簡易ランキング / 前回検証の簡易サマリー / 注意書き
...
- 各軸の「獲得点 ÷ 満点」を 0〜5 に換算したものです（バーが長いほど条件が揃っている）。
...
各銘柄カードに「**評価理由**」を**4要素・各1行**で表示します（銘柄ごとに数値・固有テーマを反映）。
詳細テキストでは同じ4要素を**3〜5文の散文**でやや詳しく示します。4要素は
①テクニカル ②出来高・流動性 ③ニュース・テーマ性 ④注意点 です。

```
評価理由（カード・4行）：
・短期・中期トレンドが上向き（5日線が25日線を+1.8%）   ← テクニカル
・出来高が20日平均を上回る（約1.4倍）で関心は上昇傾向   ← 出来高・流動性
・防衛・重工タグが、防衛・地政学関連のニュースと関連     ← ニュース・テーマ性
・一方で、テーマ先行で短期過熱になりやすい点には注意     ← 注意点
```

> 「買い」「上昇予想」などの売買推奨表現は使いません。あくまで点数の根拠を示すものです。
...
- **B. 未登録の銘柄**: JPXの業種名から**簡易プロフィールを自動生成**
  （テーマタグは業種から推定。カード上では事業内容を
  「〇〇・業種情報をもとにした簡易分類」と明示します）
...
- **横スライドカード**: 上位5銘柄を 1銘柄＝1枚のカードで横にスワイプして見られます。各カードに
  順位 / 銘柄名 / 証券コード / 総合評価（大きく表示）/ 評価ランク（High Fit〜Neutral）/ 業種 /
  事業内容 / テーマタグ / **銘柄固有のニュース環境コメント** / **評価グラフ（Flex横棒・7軸）** /
  **評価理由（4要素）** / リスクメモ（注意ボックス）/ 注意書き を掲載します
  （有料レポートを意識した、余白を取った落ち着いたデザイン）。
- **ニュース環境コメント・評価グラフ・評価理由は各銘柄カード内に必ず掲載**します（詳細テキストだけに出すことはしません）。
- 詳細テキストには、各銘柄の **評価バランス図（テキスト）** ・スコア内訳・**評価理由**・
  **ニュース環境**・リスクメモ、および **今日の世界情勢・経済ニュース** と **テーマ別の見方** を表示します。
...
| 使わない表現 | 代わりに使う表現 |
|--------------|------------------|
| おすすめ株 / 買い候補 / 買い | スクリーニング上位銘柄 / 今日見るべき銘柄 / 機械的な抽出結果 |
| 今後伸びる株 / 狙い目 / 上昇予想 | トレンド評価 / 出来高評価 / 相対強度 / テーマ性 |
| 想定上昇率 / 急騰期待 / 儲かる | ニュース環境 / 世界情勢 / 評価理由 / 過熱感 / リスクメモ |

推奨表現の例: スクリーニング上位銘柄 / 今日見るべき銘柄 / 機械的な抽出結果 / 評価理由 /
トレンド評価 / 出来高評価 / 相対強度 / テーマ性 / **ニュース環境** / **世界情勢** / 過熱感 /
リスクメモ / 売買推奨ではありません。
禁止表現: おすすめ株 / 買い候補 / 今後伸びる株 / 狙い目 / 上昇予想 / 想定上昇率 / 儲かる / 買い。
