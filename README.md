...
4. 各銘柄を **7つの評価軸** でスコアリング（合計10.0点、小数第1位まで）
   - トレンド 2.0 / 出来高・流動性 1.5 / 相対強度 1.5 / 業種テーマ性 1.5 /
     バリュエーション 1.0 / 安定性・過熱回避 1.5 / 前回検証・継続性補正 1.0
...
6. LINE送信は **①まとめカード（Flex）→ ②銘柄カルーセル（横スライドカード）→ ③詳細テキスト**
   ...
   - **②銘柄カルーセル**: 上位5銘柄を **1銘柄＝1枚のカードで横スライド表示**。各カードに
     順位 / 銘柄名 / 証券コード / 業種 / 事業内容 / **総合評価バー** / 評価ランク /
     テーマタグ / **評価バランス図（6軸）** / リスクメモ / 注意書きを掲載
     （評価点に応じて 青系＝強い・緑系＝標準・グレー系＝注意 の落ち着いた配色）
...
| 評価軸 | 配点 | 見るもの |
|--------|------|----------|
| トレンド | 2.0 | 5日・25日・75日移動平均の並び、株価との位置（5日>25日>75日なら高評価） |
| 出来高・流動性 | 1.5 | 直近出来高/20日平均の増加、売買代金（流動性が低い銘柄は減点） |
| 相対強度 | 1.5 | 市場平均（TOPIX連動ETF）に対して強いか |
| 業種テーマ性 | 1.5 | 半導体・防衛・AI 等のテーマタグ（注目テーマはやや加点） |
| バリュエーション | 1.0 | PER / PBR / 配当利回り（取得できない場合は中立） |
| 安定性・過熱回避 | 1.5 | 短期の急騰しすぎ・高ボラティリティを減点 |
| 前回検証・継続性補正 | 1.0 | 前回も上位なら加点、ただし短期過熱が強ければ減点（前回なしは中立） |
...
- 図には主要6軸（トレンド／出来高／相対強度／テーマ性／割安感／安定性）を表示し、
...
- **横スライドカード**: 上位5銘柄を 1銘柄＝1枚のカードで横にスワイプして見られます。各カードに
  順位 / 銘柄名 / 証券コード / 業種 / 事業内容 / 総合評価バー / 評価ランク / テーマタグ /
  **評価バランス図（6軸）** / リスクメモ / 注意書き を掲載します。
- **評価バランス図は各銘柄カード内に必ず掲載**します（詳細テキストだけに出すことはしません）。
- 詳細テキストには、各銘柄の **評価バランス図（テキスト）** ・スコア内訳・リスクメモ・
  「なぜ上位に入ったか」を表示します。
