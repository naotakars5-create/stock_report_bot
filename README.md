...
6. LINE送信は **①まとめカード（Flex）→ ②銘柄カルーセル（横スライドカード）→ ③詳細テキスト**
   ...
   - **②銘柄カルーセル**: 上位5銘柄を **1銘柄＝1枚のカードで横スライド表示**（有料レポートを
     ...
     順位 / 銘柄名 / 証券コード / **総合評価（大きく表示）** / 評価ランク / 業種 / 事業内容 /
     テーマタグ / **ニュース環境** / **評価バランス図（7軸）** / **評価理由** /
...
各銘柄カードのヘッダーに、総合評価（0〜10点）を 10ブロックのバーで表示します。
...
- `█` の数 ＝ 総合評価点（四捨五入）。10点満点に対して何ブロック埋まっているかで一目で強さがわかります。
- 8.5点以上は青系、7.5〜8.4点は緑系、7.4点以下はグレー系のカード配色になります。

### 評価バランス図の見方

各銘柄の評価を**テキストの簡易図**で表示します（画像ではありません）。
**LINEの銘柄カード内**と**詳細テキスト**の両方に掲載します（カードを横スライドした時点で、
その銘柄の評価バランスが一目でわかります）。
...
- 各軸の「獲得点 ÷ 満点」を 0〜5 ブロックに換算した図です（■が多いほど条件が揃っている）。
- 図には主要7軸（トレンド／出来高／相対強度／テーマ性／**ニュース**／割安感／安定性）を表示し、
  継続性補正は「スコア内訳」に含めて表示します。
- LINEの銘柄カードでは、崩れにくいように略ラベル（相対／テーマ／割安／安定）で表示します。

### 評価理由の見方（なぜその点数か）

各銘柄カード・詳細テキストに「**評価理由**」を最大4行で表示します。点数の根拠を機械的に
言語化したもので、**プラス要素**（2〜3個）／**ニュース環境との関連**（1個）／**注意点**（1個）を
組み合わせます。

```
評価理由：
・5日・25日移動平均が上向きで、短期トレンドが強い
・出来高が20日平均を上回り、市場の注目度が高い
・防衛関連テーマが市場で意識されやすい環境
・一方で、短期的な過熱感には注意
```
...
1. 見出しから市場テーマ（半導体・AI・防衛・地政学リスク・円安/円高・金利・原油 など）を抽出
2. 為替（ドル円）や米国株（S&P500/NASDAQ）の方向感も加味
3. 抽出したテーマを銘柄の `theme_tags` と照合し、関連があれば「ニュース環境」評価に反映
   - 例: 防衛ニュースが多い → 防衛タグの銘柄にプラス
   - 例: 円安進行 → 輸出関連（円安メリット）にプラス、ただし輸入コスト増業種は注意
   - 例: 原油高 → 資源・商社にプラス、運輸・電力は注意
   - 例: 金利上昇 → 銀行・保険にプラス、グロースは注意
4. 各銘柄カードに「**ニュース環境**」を1〜2行で表示し、評価バランス図にも「ニュース」軸を追加

> 表現は「意識されやすい」「関連がある」「注意」にとどめ、「買い」「上昇する」等は使いません。
...
- **横スライドカード**: 上位5銘柄を 1銘柄＝1枚のカードで横にスワイプして見られます。各カードに
  順位 / 銘柄名 / 証券コード / 総合評価（大きく表示）/ 評価ランク / 業種 / 事業内容 / テーマタグ /
  **ニュース環境** / **評価バランス図（7軸）** / **評価理由** / リスクメモ（注意ボックス）/
  注意書き を掲載します（有料レポートを意識した、余白を取った落ち着いたデザイン）。
- **評価バランス図・評価理由は各銘柄カード内に必ず掲載**します（詳細テキストだけに出すことはしません）。
